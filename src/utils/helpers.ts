...
import { BookLocal } from "@/types/types/book-local.types";
...
export function filterUniqueBooks(books: BookLocal[]) {
  const uniqueTitles = new Set();
  const uniqueBooksArray: BookLocal[] = [];

  books.forEach((book: any) => {
    if (!uniqueTitles.has(book.bookTitle)) {
      uniqueBooksArray.push(book);
      uniqueTitles.add(book.bookTitle);
    }
  });
...
export function getFormatsWithCount(books: MergedBook[]) {
  const formatCounts = {
    ...
    //PRINT_BOOK_HARDCOVER: 0,
    //PRINT_BOOK_SPECIAL_EDITION: 0,
    EBOOK: 0,
    AUDIOBOOK: 0,
  };
...
// export const mergeBooks = (books: Book[]) => books.map(book => {
//   const mergeInfo = book_formats_merge_data.find(merge => merge.id === book.id);
//   return {
//     ...book,
//     order: mergeInfo?.order || null,
//     series_name: mergeInfo?.series_name || null
//   };
...
export function getAllBookFormats() {
  ...
  const transformedBooks = booksData.flatMap(book =>
    Object.entries(book.format).map(([formatKey, formatValue]) => ({
      ...
      cover_image: `/assets/images/books/${book.image}`,

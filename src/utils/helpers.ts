...
const books: BookData[] = booksData;

/* eslint-disable @typescript-eslint/no-explicit-any */
export function filterAndConvertParams<T extends Record<string, any>>(params: T): Record<string, string> {
  ...
  if (params) {
    (Object.keys(params) as Array<keyof T>).forEach((key) => {
      ...
      if (value !== undefined && value !== null) {
        if (isDate(value)) {
          filteredParams[key as string] = value.toISOString();
        } else {
 ...
 *
 * If a search term is provided, the function filters the input array to include only those books
 * where the title contains the search term. Then, it calculates the counts based on book formats.
 *
 * The total count for "all" is determined by the number of unique book IDs in the filtered list.
 *
 * @param books - The array of merged book objects to process.
 * @param searchTerm - Optional search term used to filter the books by title.
 * @returns An array of objects, each representing a book format with its respective count.
 */
export function getFormatsWithCount(books: MergedBook[], searchTerm?: string) {
  ...
  type FormatKey = keyof typeof formatCounts;
...
export function getLocalBookById(bookId: number): BookData | undefined {
  const book = books.find((b) => b.bookId === bookId);
  return book;
}

// Return All books by series slug
...
// Return Series data by its slug 
...
export function getAllBookFormats() {
  let idCounter = 1;

  const transformedBooks = booksData.flatMap((book) =>
    Object.entries(book.format).map(([formatKey, formatValue]) => ({
      id: idCounter++,
      ...
      createDate: formatValue.createDate,
    }))
  );
...
/**
 * Get the country name based on the timezone.
 * 
...
export function getCountry(): string {
  ...
  const timezones: { [key: string]: TimezoneInfo } = {
    "Africa/Abidjan": {
      u: 0,
  ...
  // Check if the timezone exists and is valid
  ...
  const countryCode = timezoneInfo.c[0];

  // Return the country name if the country code exists
  return countryCode ? countries[countryCode] || "" : "";
}

// Genarate series Object from Books | Only use if book object updated
export function genarateSeriesData() {

  const all = new Map(books.map(item => [item.series.slug, item.series.name]));
  const uniqueSeries = Array.from(all, ([slug, name]) => ({ slug, name, description: '' }));
  const jsonObject = JSON.stringify(uniqueSeries, null, 2);
